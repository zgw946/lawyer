...
class Index extends Component {
  ...
  deleteCase(id) {
    ...
    myRequest({
      method: "put",
      path: "/lawyer/efficiency/my_case/index/"+id+"/destroy",
      auth: true,
      callback: function(response) {
        //处理返回结果
        if (response.data.code !== 0) {
  ...
  fileCase(id) {
    ...
    myRequest({
      method: "put",
      path: "/lawyer/efficiency/my_case/index/"+id+"/finish",
      auth: true,
      callback: function(response) {
        //处理返回结果
        if (response.data.code !== 0) {
                ...
                {getFieldDecorator('keyword', {})(
                  ...
                  />
					...
					<div style={{ marginTop: 20, height: "100%" }}>
						<div style={{ height: 40 }}>
							<span className="partys">在办案件</span>
							<Button
								size="small"
								className="multiadd"
								onClick={() => this.props.changePopupBox([{ type: "add_case" }])
								}
								style={{
                    ...
                    {item.describe}
									...
									>
										<div className="lawyer_box_btn pos-a">
											<div>
												<Link to={"/case_center/detail/"+item.id}>
