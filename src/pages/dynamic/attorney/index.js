...
import { ONLY_DOMAIN, DOMAIN } from "../../../constant";
...
class Index extends Component {
	...
	getLawyerDetail() {
		...
		myRequest({
			method: "get",
			path: "/lawyer/conversation/lawyer/" + conversationId,
			params: {
	...
	makeWebSocket() {
		...
		if (token) {
			let url =
				"ws://" +
				ONLY_DOMAIN +
				"/common/conversation/lawyer/open_conversation?token=" +
	...
	addList(message) {
		...
		this.setState(
			...
			() => {
				...
				if (message.member_type === 2 || bottom) {
	...
	render() {
		const {list,conversationObj} = this.state; //消息列表

		return (
			<Fragment>
				<div className="hangs">
					<p className="p_hangs">{conversationObj ? conversationObj.name : ""}</p>
				</div>
									...
									{item.map(itemChild => (
										<Fragment key={itemChild.id}>
											...
											<div
												className={
													"message_content " +
													(itemChild.member_type === 2 ? "rf mr26" : "lf ml25")
												}
											>
												{itemChild.type === 1 ? (
					...
					<div className="wires">
						<Form onSubmit={this.sendMessage}>
							...
							>
								<Icon
									type="message"
									onClick={() => this.props.changePopupBox([{ type: "history" }])}
								/>
							</div>
							{/* 聊天输入框 */}
							<Input.TextArea
