	...
	handleSubmit = e => {
		...
		this.props.form.validateFields((err, values) => {
			if (!err) {
				...
				myRequest({
					method: "post",
					path: "/lawyer/efficiency/coordination/case_agent",
					auth: true,
					data: {
					...
					callback: function(response) {
						...
						if (response.data.code === 0) {
							message.success("添加委托人协同信息成功");
							that.props.changePopupBox([{ type: "entrust" }]);
						} else {
				...
				>
					...
					<Form.Item label="标的额">
						{getFieldDecorator("amount", {
							rules: [{ required: true, message: "请输入标的额" }]
						})(<Input placeholder="请输入标的额" />)}
					</Form.Item>
					<Form.Item label="查档类型">
						{getFieldDecorator("type", {
							rules: [{ required: true, message: "请输入查档类型" }]
						})(<Input placeholder="请输入查档类型" />)}
					</Form.Item>
